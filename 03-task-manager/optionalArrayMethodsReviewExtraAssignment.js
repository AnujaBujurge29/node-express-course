...
const New_names = [
  'John Doe',
  'Jane Smith',
  'Tam  White',     // Double space
  'Carlow Daniel',  // Middle name
  'Timothy      Cook', // Multiple spaces
  'Nick_Masters',   // Contains underscore
  'Timmy-Turner',   // Contains hyphen
  'Billy\nBob'      // Contains newline character
];

// Challenge 1: Create a new array with only each person's last name
const last_Names = names.map((fullName) => {
  return fullName.split(' ').pop();
});
console.log('Last names:', lastNames);

// Challenge 2: Filter names that don't match the format "<first> <last>"
const nameFormatRegex = /^\w+\s\w+$/;
const validNames = names.filter((name) => {
  return nameFormatRegex.test(name);
});
...
const titleCaseNames = names_3.map(toTitleCase);
...
// - Last Challenge:
// Function to check if a name has the correct format
const hasCorrectFormat = (name) => {
  const nameFormatRegex = /^\w+\s\w+$/;
  return nameFormatRegex.test(name);
};

// Filter out names with the wrong format, convert to Title Case, and remove names ending with "z"
const filteredNames = names.filter((name) => {
  return hasCorrectFormat(name) && !name.toLowerCase().endsWith('z');
}).map((name) => {
  return toTitleCase(name);
});

// Write a message asking them to sign up
const signUpMessages = filteredNames.map((name) => {
  return `Hey ${name}, we'd love for you to sign up for our service!`;
});

console.log('Sign Up Messages:', signUpMessages);
